...
class CheckRewriter:
    ...
    def __init__(self, program):
        ...
        self.cost_rewriter = CostRewriter(self.program, SolverSettings.LEVEL_COST_ATOM_NAME, SolverSettings.WEAK_VIOLATION_ATOM_NAME, SolverSettings.COST_AT_LEVEL_ATOM_NAME)
        self.clone_program = ""
    ...
    def rewrite(self, create_clone=True, suffix=""):
        ...
        self.cost_program = self.cost_rewriter.rewritten_program
        ...
        if create_clone:
            ...
            cost_program_no_suffix = self.cost_rewriter.rewritten_program
            #I should rewrite to the clone signature both predicates from cost program and from the original program
            self.clone_cost_rewriter = CloneRewriter(QuantifiedProgram(cost_program_no_suffix, [], self.program.program_type, "", self.program.head_predicates | self.cost_rewriter.rewritten_program_head_predicates), f"{suffix}{SolverSettings.CLONE_ATOM_SUFFIX}")
